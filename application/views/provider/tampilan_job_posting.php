...
<!-- Bootstrap modal -->
<div class="modal fade" id="modal_status_lowongan" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body form">
        <form action="#" id="form_status_lowongan" class="form-horizontal">
          <input type="hidden" value="" name="id_lowongan" />
          <div class="form-body">
            <div class="form-group">
              <label class="control-label col-md-3">Status</label>
              <div class="col-md-9">
                <select name="lowongan_status" class="form-control">
      ...
      <div class="modal-footer">
        <button type="button" id="btnSave" onclick="save_status_lowongan()" class="btn btn-primary">Simpan</button>
...
<script type="text/javascript">
  ...
  function status_lowongan(id) {
    $('#form_status_lowongan')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
    $('.help-block').empty(); // clear error string

    //Ajax Load data from ajax
    $.ajax({
      url: "<?php echo site_url('provider/status_lowongan') ?>/" + id,
      type: "GET",
      dataType: "JSON",
      success: function(data) {

        $('[name="id_lowongan"]').val(data.lowongan_id);
        $('[name="lowongan_status"]').val(data.lowongan_status);
        $('#modal_status_lowongan').modal('show'); // show bootstrap modal when complete loaded
        $('.modal-title').text('Status Lowongan'); // Set title to Bootstrap modal title
  ...
  function save_status_lowongan() {
    $('#btnSave_status').text('sedang menyimpan...'); //change button text
    $('#btnSave_status').attr('disabled', true); //set button disable 
    var url;

    url = "<?php echo site_url('provider/update_status_lowongan') ?>";

    // ajax adding data to database
    $.ajax({
      url: url,
      type: "POST",
      data: $('#form_status_lowongan').serialize(),
      dataType: "JSON",
      success: function(data) {

        if (data.status) //if success close modal and reload ajax table
        {
          $('#modal_status_lowongan').modal('hide');
          reload_table();
          Swal.fire({
            icon: 'success',
            title: 'Berhasil',
        ...
        $('#btnSave_status').text('Simpan'); //change button text
