<div class="main-panel">
  ...
  <section class="content flashdatart" data-title="<?php echo $this->session->flashdata('title'); ?>" data-text="<?php echo $this->session->flashdata('text'); ?>" data-icon="<?php echo $this->session->flashdata('icon'); ?>">
    ...
    <div class="row">
      <div class="col-12">
        <h1 class="h3 mb-3 ml-5 text-gray-800">Master Data Agama</h1>
        <div class="card">
          <div class="card-header">
            ...
            <div class="col-sm-12"> 
             ...
             <button id="btn_tambah" class="btn btn-success btn-sm btn-md btn  mr-2" ><i class="fa fa-plus mr-2"></i> Tambah Data Agama</button>
        ...
        <div class="modal fade" data-backdrop="static" id="modal_agama" tabindex="-1" role="dialog" aria-labelledby="modal_agamaLabel" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content" >
              <div class="modal-header bg-primary text-light"> 
                <h3 class="modal-title" id="label_header_agama"> <i class="fas fa-user mr-2"></i> TAMBAH DATA USER</h3>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">x</span></button>
              </div>
              <div class="modal-body">
               <form id="form_agama" method="post" enctype="multipart/form-data" action="<?php echo base_url('agama/simpan') ?>">
                 <div class="row "> 
                  <div class="col-md-12 mb-3"> 
                   <label style="color:#343a40;" for="agama_nama">Agama</label>
                   <input type="hidden" name="agama_id" id="agama_id">
                   <input type="text" class="form-control" id="agama_nama"  name="agama_nama" required>
                 </div>   
...
<div class="modal fade" data-backdrop="static" id="modal_edit_password" tabindex="-1" role="dialog" aria-labelledby="modal_edit_passwordLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content" >
      ...
      <div class="modal-body">
        <form method="post" action="<?php echo base_url() ?>agama/reset_password">
          <div class="form-group row"class="collapse" id="customer_collapse">
            <div class="col-md-12 mb-3">
...
<div class="modal fade" data-backdrop="static" id="ModalAktivasi" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header bg-primary text-light">
       <h3 class="modal-title" id="myModalLabel" style=" font: sans-serif; "><i class="fas fa-database mr-2"></i> Status Agama</h3>
       <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">x</span></button>
     ...
     <form class="form-horizontal">
      <div class="modal-body">
        ...
        <input type="hidden" name="kode_agama_aktivasi" id="kode_agama_aktivasi" value=""> 
...
<script type="text/javascript">
  ...
  dataTable = $('#tabel_agama').DataTable( {
    ...
    ajax: {
     url: '<?php echo base_url('agama/tabel_agama')?>',
     type: 'get',
     data: function (data) {
     }
   },
 ...
 $(".refresh").click(function(){
  ...
  $('#btn_aktivasi').on('click',function(){
    var kode=$('#kode_agama_aktivasi').val();
    var isi=$('#isi_aktivasi').val();

    $.ajax({
      type : "POST",
      url  : "<?php echo base_url('agama/aktivasi')?>",
      dataType : "JSON",
  ...
  $('#show_data').on('click','.item_aktivasi_agama',function(){
    if ($(this).html().includes('check')) {
      $('.notif_aktivasi').html('Aktifkan Agama... ?');
      $('#isi_aktivasi').val(1);

    }else{
      $('.notif_aktivasi').html('Nonaktifkan Agama... ?');
      $('#isi_aktivasi').val(0);
    }

    var kode= $(this).attr('data');
    $('#ModalAktivasi').modal('show');
    $('#kode_agama_aktivasi').val(kode);

    return false;
  });

  $('#show_data').on('click','.item_edit_agama',function(){
    let agama_id = $(this).attr('data');
    $.ajax({
      type : "GET",
      url  : "<?php echo base_url('agama/detail_agama')?>",
      dataType : "JSON",
      data : {'agama_id':agama_id},
      success: function(data){

        $('#modal_agama').modal('show');
        $('#form_agama').attr('action','<?php echo base_url('agama/ubah') ?>');
        $('#btn_simpan').html('UBAH');
        $('#label_header_agama').html('<i class="fas fa-database mr-2"></i> UBAH DATA AGAMA');
        $('#agama_id').val(agama_id);
        $('#agama_nama').val(data[0].agama_nama);
      },
    });
    return false;
  });
  ...
  $('#btn_tambah').on('click',function(){
    $('#modal_agama').modal('show');
    $('#form_agama').attr('action','<?php echo base_url('agama/simpan') ?>');
    $('#btn_simpan').html('SIMPAN');
    $('#form_agama').trigger("reset");
    $('#label_header_agama').html('<i class="fas fa-database mr-2"></i> TAMBAH DATA AGAMA');
  });


  $('#btn_simpan').on('click',function(){

    let agama_nama = $('#agama_nama').val();
    if (agama_nama=="") {
      $('#agama_nama').focus();
      Swal.fire({
        title:'Agama Kosong',
        text:'Silahkan Masukkan Agama!',
        icon:'error'
      }).then((result) => {
        if (result.isConfirmed) {
          Swal.close();
